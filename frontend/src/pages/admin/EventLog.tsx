...
import { Search, Filter, Moon, Sun, MoreVertical, Menu } from 'lucide-react';
...
export default function EventLog() {
  ...
  // Real-time: refresh the event list whenever any WebSocket message arrives
  useEffect(() => {
    if (!lastMessage) return;
    
    // We fetch the latest events to ensure the table is perfectly in sync
    // This acts as a bulletproof real-time trigger for any backend state.
    getEvents({ per_page: 100 })
      .then(res => {
        setEvents(res.events);
        setTotal(res.total);
          ...
          <div className="flex-1 overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="text-sm border-b border-border/30">
                {loading ? (
                  <tr>
                    <td colSpan={7} className="p-12 text-center text-muted">Loading events...</td>
                  </tr>
                  ...
                  <tr key={evt.id} className="border-b border-border/30 hover:bg-page/50 transition-colors group">
                    ...
                    <td className="px-6 py-3 font-mono text-xs text-muted w-36 whitespace-nowrap">{new Date(evt.created_at).toLocaleString()}</td>
                    ...
                    <td className="px-6 py-3">
                      <div className="bg-page/50 rounded p-1.5 border border-border/50 text-xs font-mono text-muted overflow-hidden text-ellipsis whitespace-nowrap max-w-xs" title={JSON.stringify(evt.payload)}>
